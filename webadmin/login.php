<?php
	...
	if(!empty($_POST)){
		...
		$admindl=queryall(co_admin,"where adminuser='$zh'and adminpass='$mm'");
		if($admindl["adminuser"] !== null){
			if($admindl["sessid"] != session_id()){
				$d=session_id();
				queryg(co_admin,"sessid='$d' where adminuser='$zh'");
			}
			if($admindl["status"]=='禁用'){
				echo json_encode(array("te"=>"该管理账户已被禁用,无法登陆！"));
				exit;
			}
			$_SESSION["admin"]=$zh;
			echo json_encode(array("te"=>"登陆成功,正在为你加载系统！","ok"=>"ok"));
			exit;
		}else{
...
<head>
    ...
    <title>系统后台登陆 - <?php echo $web["sitename"]; ?></title>
            ...
            <div class="tpl-login-content">
                ...
                <form class="am-form tpl-form-line-form">
                    ...
                    <div class="am-form-group">
                        ...
                        <button id="dl" name="dl" type="button" class="am-btn am-btn-primary  am-btn-block tpl-btn-bg-color-success  tpl-login-btn"><i class='am-icon-user-secret'></i>　登录管理系统</button>
