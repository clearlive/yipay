<?php
	...
	if(@$_POST['action']=='plsc'){//批量删除订单
		...
		for($i=0;$i<count($id);$i++){
			...
			querys(co_dingdan,"where id='$id[$i]'");
		}
		echo json_encode(array("te"=>"删除完成！","ok"=>"ok"),true);
		exit;
	}
...
<head>
    ...
    <title>订单记录 - 系统后台 -<?php echo $web["sitename"]; ?></title>
                            ...
                            <div class="widget-body  am-fr">
                                <div class="am-scrollable-horizontal ">
                                    <table width="100%" class="am-table am-table-compact am-text-nowrap am-table-striped tpl-table-black " id="example-r">
                                        ...
                                        <tbody>
										<?php
											$zs=querydb(co_dingdan,"where username !='codiaodan'");//总条数
											$my=6;//每页显示多少条
											$zys=ceil($zs/$my);//总页数
											...
											$djt=($t-1)*$my;//第几条开始
											$sql=mysql_query("select * from co_dingdan where username !='codiaodan' ORDER BY id DESC limit $djt,$my");
											$i=$djt+1;
											while($ddjl=mysql_fetch_assoc($sql)){
												if($ddjl['ddzt']=='success'){
													$ddjl['ddzt']='<a class="am-badge am-badge-success am-radius">成功</a>';
												}else if($ddjl['ddzt']=='fail'){
													$ddjl['ddzt']='<a class="am-badge am-badge-danger am-radius">待付</a>';
												}else if($ddjl['ddzt']=='wait'){
													$ddjl['ddzt']='<a class="am-badge am-badge-warning am-radius">等待</a>';
												}
												if($ddjl['username']=='codiaodan'){
													$ddjl['username']="<b style='color:red'>概率掉单</b>";
												}
												echo '
											<tr class="gradeX">
                                                <td><input type="checkbox" class="a" name="x" value="'.$ddjl['id'].'">'.$i.'</td>
                                                <td>'.$ddjl['username'].'</td>
                                                <td>'.$ddjl['ddh'].'</td>
                                                <td>'.$ddjl['ddsj'].'</td>
                                                <td>'.$ddjl['ddtdmc'].'</td>
                                                ...
                                                <td>
                                                    <div class="tpl-table-black-operation">
                                                        <span onclick="ck(this)" val="'.$ddjl['id'].'"><a href="javascript:;" class="tpl-table-black-operation-info">
                                                            <i class="am-icon-eye"></i> 查看
                                                        </a></span>
                                                        <span onclick="sc(this)" val="'.$ddjl['id'].'"><a href="javascript:void(0)" class="tpl-table-black-operation-del">
