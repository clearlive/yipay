<?php
	...
	if(@$_POST['action']=='jiesuan'){//结算
		...
		$dljs=queryall(co_agenttx,"where id='$id'");//代理的数据
		$yhm=$dljs['agentuser'];//获取代理名
		$dqtime=date("Y-m-d H:i:s",time());//当前时间
		queryg(co_agenttx,"txzt='yes' where id='$id'");//修改结算状态
	...
	if(@$_POST['action']=='pljs'){//批量结算
		...
		$id=$_POST["id"];
		$dljs=queryall(co_agenttx,"where id='$id[0]'");//代理的数据
		$yhm=$dljs['agentuser'];//获取代理名
		$dqtime=date("Y-m-d H:i:s",time());//当前时间
		for($i=0;$i<count($id);$i++){
			$id[$i]=uhtml(check(trim($id[$i])));
			queryg(co_agenttx,"txzt='yes' where id='$id[$i]'");//修改结算状态
		}
		echo json_encode(array("te"=>"结算处理完成！","ok"=>"ok"),true);
		exit;
	}
...
<head>
    ...
    <title>代理结算 - <?php echo $web["sitename"]; ?></title>
            ...
            <div class="container-fluid am-cf">
                <div class="row">
                    <div class="am-u-sm-12 am-u-md-12 am-u-lg-9">
                        <div class="page-header-heading"><span class="am-icon-home page-header-heading-icon"></span> 代理结算 <small>v 2.0</small></div>
                            ...
                            <div class="widget-body  am-fr">
                                <div class="am-scrollable-horizontal ">
                                    <table width="100%" class="am-table am-table-compact am-text-nowrap am-table-striped tpl-table-black " id="example-r">
                                        ...
                                        <tbody>
										<?php
											$zs=querydb(co_agenttx,"where txzt='no'");//总条数
											$my=6;//每页显示多少条
											$zys=ceil($zs/$my);//总页数
											...
											$djt=($t-1)*$my;//第几条开始
											$sql=mysql_query("select * from co_agenttx where txzt='no' ORDER BY id DESC limit $djt,$my");
											$i=$djt+1;
											while($dljs=mysql_fetch_assoc($sql)){
												echo '
											<tr class="gradeX">
                                                <td><input type="checkbox" class="a" name="x" value="'.$dljs['id'].'">'.$i.'</td>
                                                <td>'.$dljs['agentuser'].'</td>
                                                <td>'.$dljs['txje'].'</td>
                                                <td>'.$dljs['sxf'].'</td>
                                                <td>'.$dljs['sjje'].'</td>
                                                <td>'.$dljs['zsxm'].'</td>
                                                ...
                                                <td>
                                                    <div class="tpl-table-black-operation">
                                                        <span onclick="js(this)" val="'.$dljs['id'].'"><a href="javascript:void(0)" class="tpl-table-black-operation-success">
			...
			onConfirm:function(){
				$.ajax({
					...
					success:function(data){
						...
						if(data.ok){
							setTimeout(function(){
								window.location='dc.php?lx=dljs&action=pljs&id='+id;
							},1000);
							setTimeout(function(){
			...
			onConfirm:function(){
				$.ajax({
					...
					success:function(data){
						...
						if(data.ok){
							setTimeout(function(){
								window.location='dc.php?lx=dljs&action=js&id='+$(obj).attr("val");
							},1000);
							setTimeout(function(){
								window.location.reload();
							},3000);
